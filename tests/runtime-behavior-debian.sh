...
#
# Purpose: Runtime behavior test for Debian Docker container
# Usage: sh tests/runtime-behavior-debian.sh
#
# Requirements: 3.2, 4.4, 5.1-5.10
# Validates:
#   - Default root user configuration
#   - Custom user creation with PUID/PGID/USER
#   - Environment variables are set correctly (including DEBIAN_FRONTEND)
...
# ── Configuration ────────────────────────────────────────────────────────────
TEST_IMAGE_NAME="base-debian-runtime-test"
...
test_build_default_image() {
  log_info "Building Debian Docker image with default configuration..."

  if ! docker build -t "${TEST_IMAGE_NAME}:${DEFAULT_IMAGE_TAG}" -f "${DOCKERFILE_PATH}" docker/debian/ >/dev/null 2>&1; then
...
test_build_custom_user_image() {
  log_info "Building Debian Docker image with custom user configuration..."
  ...
  if ! docker build \
    ...
    -f "${DOCKERFILE_PATH}" \
    docker/debian/ >/dev/null 2>&1; then
...
# Purpose: Test DEBIAN_FRONTEND environment variable
test_debian_frontend() {
  log_info "Testing DEBIAN_FRONTEND environment variable..."

  local debian_frontend_value
  debian_frontend_value=$(docker run --rm "${TEST_IMAGE_NAME}:${DEFAULT_IMAGE_TAG}" \
    sh -c 'echo $DEBIAN_FRONTEND' 2>&1)

  if [ "${debian_frontend_value}" != "noninteractive" ]; then
    log_error "DEBIAN_FRONTEND environment variable is incorrect"
    log_info "  Expected: DEBIAN_FRONTEND=noninteractive"
    log_info "  Actual:   DEBIAN_FRONTEND=${debian_frontend_value}"
    return 1
  fi

  log_success "DEBIAN_FRONTEND environment variable is correct: noninteractive"
  return 0
}
...
main() {
  ...
  log_info "=== Debian Docker Container Runtime Behavior Test ==="
  echo ""
  ...
  test_debian_frontend || errors=$((errors + 1))
  echo ""
  ...
  if [ "${errors}" -eq 0 ]; then
    log_success "All tests passed! Debian Docker container runtime behavior verification successful."
    echo ""
    log_info "Verified:"
    ...
    printf "  ✓ DEBIAN_FRONTEND environment variable is correct\n"
