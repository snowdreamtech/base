...
#
# Purpose: Build verification test for Rocky Docker image
# Usage: sh tests/build-verification-rocky.sh
#
...
DOCKERFILE_PATH="rocky/Dockerfile"
...
test_build_image() {
  log_info "Building Rocky Docker image..."

  if ! docker build -t "${TEST_IMAGE_NAME}:${TEST_IMAGE_TAG}" -f "${DOCKERFILE_PATH}" rocky/ 2>&1; then
...
main() {
  ...
  log_info "=== Rocky Docker Image Build Verification Test ==="
  echo ""
  ...
  if [ "${errors}" -eq 0 ]; then
    log_success "All tests passed! Rocky Docker image build verification successful."
    echo ""
    log_info "Verified:"
